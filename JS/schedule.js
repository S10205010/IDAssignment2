...
function displayTasklist(task, weaFc24Hr, forecast) {
  ...
  for (a = 0; a < task.length; a++) {
    ...
    $("#tasks").append(
      `<div id="${key}"><h3>Event: ${currentTask.eventName} <span>(${currentTask.eventDate})</span></h3></div>`
    );
    $("#" + key).append(`<div class = 'row'id ="${keyEvent}"></div>`);
    $("#" + keyEvent).append(`
    <div class = 'col-sm'>Location: ${
      area[currentTask.locationIndex].name
    }</div>
    <div class = 'col-sm'>Time: (${currentTask.startTime}-${
      currentTask.endTime
    })</div>`);
...
$(document).ajaxStop(function () {
  ...
  $("#button").click(function (event) {
    ...
    if (eventName.length == 0) {
      ...
      $(".error").append(
        "<div>Error, Start time and End Time input error!</div>"
      );
    } else {
      //create object
